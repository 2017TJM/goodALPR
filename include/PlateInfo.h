...
namespace pr {
    ...
    class PlateInfo {
        ...
        public:
            ...
            PlateInfo(const cv::Mat &plateData, cv::Rect plateRect, std::string fileName) {
                licensePlate = plateData;
                ROI = plateRect;
            ...
            std::string getPlateName() {
                return name;

            }
            void setPlateName(std::string plateName) {
                name = plateName;
            }
            int getPlateType() {
                return Type;
...
//                        std::cout<<*std::max_element(prob+31,prob+65)<<std::endl;
			...
			void setFileName(std::string str)//新加
			{
				plateFileName = str;
			}

			std::string getFileName()//新加
			{
